...
func platformRoute(fqdn string, gen int64, ip string) model.Route {
	return model.Route{FQDN: fqdn, DesiredState: model.Present, Generation: gen, TargetIP: ip, TargetPort: 8080, CertRef: model.CertRefWildcardPrefix + "pusan.dev"}
}

func TestApplyPresentRendersAndReloads(t *testing.T) {
	h := newHarness(t)
	code, res := h.mgr.Apply(context.Background(), platformRoute("a.pusan.dev", 3, "172.29.4.11"))
	if code != 200 || !res.Applied || res.Generation != 3 {
		t.Fatalf("apply => %d %+v", code, res)
	}
	if !strings.Contains(h.readConf(t, "a.pusan.dev"), "proxy_pass http://172.29.4.11:8080;") {
...
func TestApplyStaleRejected(t *testing.T) {
	h := newHarness(t)
	if code, _ := h.mgr.Apply(context.Background(), platformRoute("a.pusan.dev", 5, "172.29.4.11")); code != 200 {
		t.Fatalf("first apply code %d", code)
	}
	// Equal generation is stale.
	code, res := h.mgr.Apply(context.Background(), platformRoute("a.pusan.dev", 5, "172.29.4.99"))
	...
	code, res = h.mgr.Apply(context.Background(), platformRoute("a.pusan.dev", 4, "172.29.4.99"))
	...
	if !strings.Contains(h.readConf(t, "a.pusan.dev"), "172.29.4.11") {
		t.Fatal("stale apply clobbered the live target")
	}
	// A newer generation is accepted.
	if code, _ := h.mgr.Apply(context.Background(), platformRoute("a.pusan.dev", 6, "172.29.4.22")); code != 200 {
...
func TestApplyAbsentRemovesButKeepsGeneration(t *testing.T) {
	h := newHarness(t)
	_, _ = h.mgr.Apply(context.Background(), platformRoute("gone.pusan.dev", 2, "172.29.4.11"))

	code, res := h.mgr.Apply(context.Background(), model.Route{FQDN: "gone.pusan.dev", DesiredState: model.Absent, Generation: 3})
	if code != 200 || !res.Applied {
		t.Fatalf("absent => %d %+v", code, res)
	}
	if _, err := os.Stat(h.confPath("gone.pusan.dev")); !os.IsNotExist(err) {
		t.Fatal("ABSENT should have removed the vhost file")
	}
	// A stale PRESENT (gen ≤ 3) must not resurrect the vhost onto a reused IP.
	code, _ = h.mgr.Apply(context.Background(), platformRoute("gone.pusan.dev", 3, "172.29.9.99"))
	if code != 409 {
...
func TestApplyNginxTestFailureLeavesLiveConfigUntouched(t *testing.T) {
	...
	code, res := h.mgr.Apply(context.Background(), platformRoute("a.pusan.dev", 2, "172.29.4.222"))
	if code != 422 || res.Applied || !strings.Contains(res.Error, "emerg") {
		t.Fatalf("failed apply => %d %+v", code, res)
	}
	if got := h.readConf(t, "a.pusan.dev"); !strings.Contains(got, "172.29.4.11") || strings.Contains(got, "172.29.4.222") {
	...
	if gen, _ := h.st.Generation("a.pusan.dev"); gen != 1 {
...
func TestApplyNewFqdnFailureRemovesTempFile(t *testing.T) {
	h := newHarness(t)
	h.ng.FailTest = true
	code, _ := h.mgr.Apply(context.Background(), platformRoute("new.pusan.dev", 1, "172.29.4.11"))
	if code != 422 {
		t.Fatalf("expected 422, got %d", code)
	}
	if _, err := os.Stat(h.confPath("new.pusan.dev")); !os.IsNotExist(err) {
...
func TestSyncAllPrunesStaleFiles(t *testing.T) {
	...
	gen, known := h.st.Generation("stale.pusan.dev")
	if !known || gen != 1 {
...
func TestSyncAllDoesNotRegressNewerGeneration(t *testing.T) {
	...
	_, _ = h.mgr.Apply(context.Background(), platformRoute("race.pusan.dev", 5, "172.29.4.11"))
	...
	code, res := h.mgr.SyncAll(context.Background(), model.SyncAllRequest{SnapshotGeneration: 30, Routes: []model.Route{
		platformRoute("race.pusan.dev", 3, "172.29.4.99"),
	}})
	if code != 200 || !res.Applied {
		t.Fatalf("sync-all => %d %+v", code, res)
	}
	// The newer applied vhost must stay live, unpruned, at gen 5.
	if got := h.readConf(t, "race.pusan.dev"); !strings.Contains(got, "172.29.4.11") || strings.Contains(got, "172.29.4.99") {
		t.Fatalf("sync-all regressed the vhost to the older manifest:\n%s", got)
	}
	if gen, _ := h.st.Generation("race.pusan.dev"); gen != 5 {
...
func TestSyncAllPrunedFqdnStillRejectsStalePresent(t *testing.T) {
	h := newHarness(t)
	_, _ = h.mgr.Apply(context.Background(), platformRoute("reused.pusan.dev", 4, "172.29.4.11"))
	...
	if _, err := os.Stat(h.confPath("reused.pusan.dev")); !os.IsNotExist(err) {
		t.Fatal("sync-all did not prune the vhost")
	}
	// A late stale PRESENT (gen ≤ 4) must not resurrect the vhost onto a reused IP.
	code, res := h.mgr.Apply(context.Background(), platformRoute("reused.pusan.dev", 4, "172.29.9.99"))
	...
	if code, _ := h.mgr.Apply(context.Background(), platformRoute("reused.pusan.dev", 5, "172.29.4.22")); code != 200 {
...
func TestSyncAllFailureLeavesTreeUntouched(t *testing.T) {
	...
	code, _ := h.mgr.SyncAll(context.Background(), model.SyncAllRequest{SnapshotGeneration: 20, Routes: []model.Route{
		platformRoute("other.pusan.dev", 1, "172.29.4.30"),
	}})
	if code != 422 {
		t.Fatalf("expected 422, got %d", code)
	}
	// The prior tree must be exactly restored: keep present, other absent.
	if _, err := os.Stat(h.confPath("keep.pusan.dev")); err != nil {
		t.Fatal("sync-all failure dropped the prior vhost")
	}
	if _, err := os.Stat(h.confPath("other.pusan.dev")); !os.IsNotExist(err) {

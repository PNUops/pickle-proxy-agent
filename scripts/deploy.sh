...
# PICKLE_PROXY_AGENT_SITE_LIMITS=off until they are declared).
...
# PICKLE_PROXY_AGENT_SITE_LIMITS is written on: a target whose base nginx config does
# not declare the rate-limit zones yet needs it set to off before the service starts,
# or the first apply fails `nginx -t`.
# shellcheck disable=SC2086,SC2029
ssh $SSH_OPTS "$TARGET" "test -f $ENV_DIR/agent.env || { umask 077 && printf '%s\n' \
  ...
  'PICKLE_PROXY_AGENT_SITE_LIMITS=on' \
  > $ENV_DIR/agent.env; }"
